...
const oasis = (history: number[], lastValue)=> {
    ...
    while(historSum !== 0) {
        ...
        const lastOasisResult = oasis(nonZero, history[history.length-1])
        history.push(lastOasisResult[lastOasisResult.length-1]+history[history.length-1])
        return history
    }
...
input.forEach(history => {
    ...
    const oResult = oasis(numberVals, numberVals[numberVals.length-1])
